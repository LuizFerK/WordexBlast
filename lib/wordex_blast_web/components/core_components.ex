defmodule WordexBlastWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-medium leading-6">
        ...
        <.icon
          :if={@kind == :error}
          name="hero-exclamation-circle-mini mb-[2px]"
          class="h-4 w-4 text-rose-900"
        />
        <%= @title %>
      </p>
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-slate-50">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(WordexBlastWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(WordexBlastWeb.Gettext, "errors", msg, opts)
    end
  end
